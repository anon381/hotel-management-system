  ...
  const content = (
    <div className="flex flex-col h-full">
        ...
        <AnimatePresence>
          {!collapsed && (
            <motion.div initial={{ opacity: 0, width: 0 }} animate={{ opacity: 1, width: "auto" }} exit={{ opacity: 0, width: 0 }} className="overflow-hidden">
              <h1 className="font-display text-lg font-bold text-foreground whitespace-nowrap">Kitchen</h1>
              <p className="text-[10px] text-muted-foreground whitespace-nowrap">Staff Portal</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <div className="p-4 border-t border-border/30 space-y-3">
        ...
        <button onClick={() => { localStorage.removeItem("token"); localStorage.removeItem("user"); navigate("/login/kitchen"); }} className="flex items-center justify-center gap-2 w-full py-2 rounded-lg text-muted-foreground hover:text-destructive hover:bg-muted/50 transition-colors text-sm">
      ...
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            <motion.aside initial={{ x: -280 }} animate={{ x: 0 }} exit={{ x: -280 }} transition={{ type: "spring", damping: 25, stiffness: 300 }} className="lg:hidden fixed left-4 top-4 bottom-4 w-[260px] bg-background/95 backdrop-blur-xl border border-border/30 z-50 shadow-2xl rounded-2xl">
              {content}
            </motion.aside>
          </>
        )}
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      ...
      <main className="flex-1 min-w-0 w-full">
        ...
        <div className="px-3 py-3 sm:p-6 lg:p-8">{children}</div>
      </main>
    </div>
  );
}
