...
export default function AdminDashboard() {
  return (
    <AdminLayout>
      <PageHeader title="Admin Dashboard" subtitle="Welcome back, Admin. Here's today's overview." />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="lg:col-span-2 glass-card-elevated overflow-hidden">
          <div className="p-5 border-b border-border/50 flex items-center justify-between">
            <h2 className="font-display font-semibold text-lg text-foreground">Recent Orders</h2>
            <motion.button whileHover={{ x: 4 }} className="text-sm text-primary flex items-center gap-1 font-medium">View All <ArrowUpRight className="w-3.5 h-3.5" /></motion.button>
          </div>
          <motion.div variants={container} initial="hidden" animate="show">
            {recentOrders.map((order) => (
              <motion.div key={order.id} variants={item} className="flex items-center justify-between p-4 hover:bg-muted/30 transition-colors border-b border-border/30 last:border-0">
                ...
                <div className="flex items-center gap-3 flex-shrink-0 ml-3">
                  <span className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${statusColors[order.status]}`}>{order.status}</span>
        ...
        <div className="space-y-6">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass-card-elevated p-5">
            ...
            <div className="space-y-3">
              {topItems.map((ti, i) => (
                <motion.div key={ti.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.5 + i * 0.08 }} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <span className="w-6 h-6 rounded-full gradient-warm text-[11px] font-bold text-primary-foreground flex items-center justify-center">{i + 1}</span>
                    <div>
                      <p className="text-sm font-medium text-foreground">{ti.name}</p>
                      <p className="text-[11px] text-muted-foreground">{ti.orders} orders</p>
                    </div>
                  </div>
                  <span className="text-xs font-semibold text-success">{ti.trend}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="glass-card-elevated p-5">
            ...
            <div className="space-y-3">
              {lowStock.map((s, i) => (
                <motion.div key={s.name} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 + i * 0.08 }} className="flex items-center justify-between">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="glass-card-elevated p-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-info/10 rounded-lg p-3 text-center"><p className="text-2xl font-display font-bold text-info">7</p><p className="text-[11px] text-muted-foreground">Preparing</p></div>
              <div className="bg-success/10 rounded-lg p-3 text-center"><p className="text-2xl font-display font-bold text-success">3</p><p className="text-[11px] text-muted-foreground">Ready</p></div>
              <div className="bg-warning/10 rounded-lg p-3 text-center"><p className="text-2xl font-display font-bold text-warning">5</p><p className="text-[11px] text-muted-foreground">Pending</p></div>
              <div className="bg-muted rounded-lg p-3 text-center flex items-center justify-center gap-1"><Clock className="w-3.5 h-3.5 text-muted-foreground" /><p className="text-sm font-semibold text-muted-foreground">~12m</p></div>
            </div>
          </motion.div>
        </div>
      </div>
